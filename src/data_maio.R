...
funcoes_mai_23 <- funcoes_mai_23 |> filter( stringr::str_detect(funcoes_mai_23$`Nível Função2`, '^(FCE)|(CCE)'))  |> 
  mutate(
  Etnia = case_when( 
...
#funcoes_mai_23 |> filter( stringr::str_detect(funcoes_mai_23$`Nível Função2`, '^(FCE)|(CCE)')) |> View()


funcoes_mai_23 |> group_by(nome_cor_origem_etnica) |>
 dplyr::summarise(
  total = sum(quantidade_de_vinculos_cargos_e_funcoes))
