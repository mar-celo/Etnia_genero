...
dados <- readRDS("data/Tab.rds") |> filter(Órgão != "Agencia Brasileira De Inteligencia") |> dplyr::rename(`Natureza Juridica` = natureza_juridica,
                                 `Raça/Cor` = etnia, 
                                 `%` = `% Cargo-Função/Etnia`)
...
```{r pizza}
data_funcao <- dados |> select(`Natureza Juridica`,
                               `Órgão Superior`,
                               Órgão, `Raça/Cor`, `Cargo-Função`, Fem, Mas, Total) |> 
  tidyr::pivot_wider(names_from = `Cargo-Função`, values_from = Total) |> 
  filter(Órgão != "Agencia Brasileira De Inteligencia")

# data_funcao1 <- data_funcao |>  
#   filter(Etnia == "Demais Raça/Cor") |> janitor::adorn_totals(where = "row", fill = "(All)")
# 
# data_funcao1$Etnia <- stringr::str_replace(data_funcao1$Etnia, "\\(All\\)", "Demais Raça/Cor")
# 
# data_funcao2 <- data_funcao |>  
#   filter(Etnia == "Negras") |> janitor::adorn_totals(where = "row", fill = "(All)")
# 
# data_funcao2$Etnia <- stringr::str_replace(data_funcao2$Etnia, "\\(All\\)", "Negras")
# 
# data_funcao <- dplyr::bind_rows(data_funcao1, data_funcao2) 
  ...
  DT::datatable(
    ...
    options = list(
      ...
      columnDefs = list(
        list(
          visible = FALSE,
          targets = c("Natureza Juridica",  "Órgão Superior")
        )
      ),
      buttons = list(
