---
title: 'Painel de Monitoramento: Diversidade Étnico-Racial em Cargos Comissionados'
output: 
  html_document: 
    toc: true
...
sd_total <- SharedData$new(dados, group = "orgao_vinc")
...
Tab_ind3 <- readRDS("../data/Tab_ind3.rds")  %>%
  .[,indice_necessidade :=  round(indice_necessidade,2)] %>%
  arrange(-indice_necessidade) %>%
  rename(
    ...
    `Razao de vagas necessarias` = indice_necessidade
  ) |> 
  filter(Orgao != "Agencia Brasileira De Inteligencia")


# setando a mesma ordem dos dados
Tab_ind3 <- 
  left_join(
    Tab_ind3,
    dados %>%
...
sd_ind3 <- SharedData$new(Tab_ind3, group = "orgao_vinc_necessidade")
...
Tab_ind4_orgaos <- readRDS( "../data/Tab_inds_4_orgaos.rds") %>% 
  ...
  rename(
    ...
    `Orgao` = orgao_vinculado_cargos_e_funcoes,
    # `Cor-origem etnica` = nome_cor_origem_etnica,
    `Percentual efetivos` = qtde,
    `Razao equidade, niveis 1 a 12` = ind4_1_a_12,
    `Razao equidade, niveis 13 a 17` = ind4_13_a_17
  )
  
setnames(Tab_ind4_orgaos,
         gsub("N.v","Niv",names(Tab_ind4_orgaos))
         )
...
# **Indicador 1: Participação de pessoas negras em cargos CCE/FCE (níveis 1 a 12 e níveis 13 a 17)**


## **Evolução mensal do percentual de Pessoas Negras (Pretas/Pardas) em Cargos Comissionados na Administração Pública Federal**
...
dt2_sup <- sd_sup_total |> 
  ...
  formatStyle(
    'Nivel 1 a 12',
    background = styleEqual(dom_sup_1a12,color_fun(dom_sup_1a12)),
  ...
  formatStyle(
    'Nivel 13 a 17',
    background = styleEqual(dom_sup_13a17,color_fun(dom_sup_13a17)),
    backgroundSize = '98% 88%',
    backgroundRepeat = 'no-repeat',
    backgroundPosition = 'center'
  )
...
dt2 <- sd_total |> 
  ...
  formatStyle(
    'Nivel 1 a 12',
    background = styleEqual(dom_1a12,color_fun(dom_1a12)),
    backgroundSize = '98% 88%',
    backgroundRepeat = 'no-repeat',
    backgroundPosition = 'center'
  ) %>%
  formatStyle(
    'Nivel 13 a 17',
    background = styleEqual(dom_13a17,color_fun(dom_13a17)),
    backgroundSize = '98% 88%',
    backgroundRepeat = 'no-repeat',
    backgroundPosition = 'center'
  )
...
# **Indicador 2: Índice de suficiência de vagas para cumprimento da meta racial**


```{r prep_necessidade_vagas}

# tbela necessidade de vagas
lim <- Tab_ind3[is.finite(`Razao de vagas necessarias`) & 
                  `Razao de vagas necessarias` < 99,
                max(abs(`Razao de vagas necessarias` - 1),na.rm = T)]

pal <- col_numeric(
  palette = c(pal_primaria["principal"],"white",pal_primaria['ponto']),
  domain = c(1 - lim, 1 + lim)
)

dom_nece <- Tab_ind3[,`Razao de vagas necessarias`]
...
dt3 <- sd_ind3 |> 
  ...
  formatStyle(
    'Razao de vagas necessarias',
    background = styleEqual(dom_nece,pal(dom_nece)),
    backgroundSize = '98% 88%',
    backgroundRepeat = 'no-repeat',
    backgroundPosition = 'center'
  )
...
#  **Indicador 4: :razão entre remuneração média dos CCEs e FCEs negros em relação á observada entre os brancos**


## **Evolução mensal do Índice de equidade remuneratória dos cargos CCE e FCE **
...
gf_medias <- Tab_ind5_mes  %>%
  ...
  geom_line() + 
  geom_point(size = 2.2) + 
  labs(x = "",col = "",y = "",title = "Remuneração média")
...
gf_ind <- Tab_ind5_mes  %>%
  ...
  geom_line() + 
  geom_point(size = 2.2) + 
  labs(x = "",col = "",y = "",title = "Índice de equidade remuneratória")


# 
# bscols(ggplotly(gf_medias),
#        ggplotly(gf_ind),
#        widths = c(6,6),
#        device = "sm") 
```
...
## **Índice de equidade remuneratória dos cargos CCE e FCE por órgão **
...
bscols(ggplotly(gf_ind),
       ggplotly(plot_cce_nivel_etna),
       widths = c(6,6),
       device = "sm")
